...
---
## Application : Utiliser un rôle communautaire depuis Ansible Galaxy

Jusqu'ici, vous avez créé vos propres rôles à la main. Ansible Galaxy permet
d'aller plus vite en réutilisant des rôles déjà écrits et maintenus par la
communauté. Cette application vous fait manipuler ce flux de bout en bout.

**Consignes :**

1. Recherchez sur Ansible Galaxy un rôle permettant d'installer et de
   configurer **Redis** :
   ```bash
   ansible-galaxy search redis
   ansible-galaxy info geerlingguy.redis
   ```
2. Installez ce rôle dans un répertoire `roles/` dédié à votre projet plutôt
   que dans le répertoire par défaut de l'utilisateur, à l'aide d'un fichier
   `requirements.yml` :
   ```yaml
   ---
   roles:
   ...
   ```
3. Créez un playbook `redis.yml` qui utilise ce rôle sur le groupe `db`, en
   surchargeant au moins deux variables par défaut du rôle (par exemple le
   port d'écoute et la limite mémoire) directement dans le playbook :
   ```yaml
   ---
   - name: Installation de Redis via un rôle Galaxy
     hosts: db
     become: yes
     roles:
   ...
   ```
4. Exécutez le playbook et vérifiez que Redis tourne bien sur le port
   personnalisé :
   ```bash
   ansible db -a "systemctl status redis"
   ansible db -a "ss -tlnp | grep 6380"
   ```
5. Listez enfin les rôles installés dans votre projet :
   ```bash
   ansible-galaxy list -p ./roles
   ```

**Critères de réussite :**
- ✅ Le rôle `geerlingguy.redis` est installé localement dans `./roles`
  (traçable via `requirements.yml`, pas dans `~/.ansible/roles`)
- ✅ Redis est fonctionnel sur `db` avec le port et la limite mémoire
  personnalisés, sans avoir modifié le code du rôle lui-même
- ✅ Vous savez expliquer la différence entre surcharger une variable d'un
  rôle Galaxy et modifier directement ses fichiers `defaults/main.yml`
