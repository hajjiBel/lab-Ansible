...
4. Auth Method : **Webhook GitHub** (validation par secret partagé — c'est le
   mécanisme natif que GitHub utilise pour signer et authentifier ses
   requêtes de webhook)
...
## ✅ Application 3 : créer un template pour la gestion des utilisateurs

En reprenant le rôle `user-management` (fil rouge Gestion des utilisateurs et
accès SSH, Labs 4 à 8) :

1. Ajoutez la structure `roles/user-management/` ainsi que le playbook
   `site_users.yml` à votre dépôt `playbooks-formation`
2. Dans Semaphore, créez un nouveau Task Template :
   - Name : `Gestion des utilisateurs`
   - Playbook Filename : `site_users.yml`
   - Repository : `playbooks-formation`
   - Inventory : `inventaire-lab`
3. Exécutez le template depuis l'interface
4. Vérifiez le résultat directement depuis `master` :
   ```bash
   ansible webservers -a "id alice"
   ansible webservers -a "id bob"
   ansible webservers -a "getent group sudo"
   ```
5. Confirmez qu'`alice` et `carla` appartiennent bien au groupe `sudo`, alors
   que `bob` n'y figure pas

**Critères de réussite :**
- ✅ Le job `Gestion des utilisateurs` s'exécute avec succès depuis
  Semaphore, visible dans **History**
- ✅ Les comptes `alice`, `bob` et `carla` existent sur `app1` et `app2`
- ✅ Seuls `alice` et `carla` (rôle `admin`) appartiennent au groupe `sudo`
...
- Vérifiez que le **Secret** est **identique, sans espace superflu**, des deux
  côtés (GitHub et Integration Semaphore) — une signature invalide sur le
  webhook GitHub entraîne un rejet automatique de la requête
